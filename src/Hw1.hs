...
doubleEveryOther xs = doubleHelper xs 0

doubleHelper :: [Integer] -> Integer -> [Integer]
doubleHelper [] _ = []
doubleHelper (x:xs) n
    |even n = x : doubleHelper xs (n+1)
    |otherwise = x*2 : doubleHelper xs (n+1)
...
validateCardNumber n
    |mod (sumList (digitsOfInts (doubleEveryOther (listReverse (digitsOfInt n))))) 10 == 0 = True
    |otherwise = False